...
router.post('/chat', async (req, res) => {
    ...
    try {
        ...
        if (requestData.birthInfo || (requestData.type === 'bazi' && requestData.context?.includes('birth'))) {
            try {
                console.log('🔮 调用@cantian-ai/Bazi-MCP服务（聊天模式）...');
                // 提取或使用提供的生辰数据
                const birthData = requestData.birthInfo || extractBirthDataFromContext(requestData.context);
                if (birthData) {
                    const baziResult = await mcpService.calculateBazi(birthData);
                    if (baziResult.success) {
                        baziData = baziResult.data;
...
//# sourceMappingURL=fortune_clean.js.map