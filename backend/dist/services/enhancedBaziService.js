...
exports.EnhancedBaziService = void 0;
const axios_1 = __importDefault(require("axios"));
class EnhancedBaziService {
    constructor() {
        // 使用可用的八字分析API端点
    ...
    async callExternalMCP(request) {
        try {
            ...
            const response = await axios_1.default.post(this.mcpEndpoint, mcpRequest, {
                headers: {
     ...
     * 生成智能八字分析
     */
    generateIntelligentBaziAnalysis(request) {
     ...
     * 生成基于上下文的分析
     */
    generateContextualAnalysis(questionType, context, question) {
        const baseAnalyses = {
            career: [
        ...
        const analyses = baseAnalyses[questionType] || baseAnalyses.general;
        // 根据上下文调整分析内容
     ...
     * 根据上下文调整分析
     */
    adjustAnalysisForContext(baseAnalysis, context) {
     ...
     * 添加时间相关的分析
     */
    addTimeSpecificAnalysis(questionType) {
        const now = new Date();
        ...
        const monthAdvice = monthAnalysis[month];
     ...
     * 生成个性化建议
     */
    generatePersonalizedAdvice(questionType) {
        ...
        const adviceList = adviceTemplates[questionType] || adviceTemplates.general;
        return adviceList[Math.floor(Math.random() * adviceList.length)];
    }
    /**
     * 生成幸运元素
     */
    generateLuckyElements(questionType) {
        ...
        const elements = elementTemplates[questionType] || elementTemplates.general;
        // 随机选择5个元素
     ...
     * 增强预测内容
     */
    enhancePrediction(baseAnalysis, question) {
     ...
     * 格式化幸运元素
     */
    formatLuckyElements(elements) {
        const defaultElements = ['绿色', '东方', '木属性饰品', '6', '8', '星期四'];
        if (!elements || elements.length === 0) {
            return defaultElements;
        }
        return elements.map(element => typeof element === 'string' ? element : String(element)).filter((element, index, array) => array.indexOf(element) === index);
    }
    /**
     * 检查服务健康状态
     */
    async checkServiceHealth() {
        try {
            const response = await axios_1.default.get(this.mcpEndpoint.replace('/bazi-analysis', '/health'), {
                timeout: 5000
            });
...
exports.EnhancedBaziService = EnhancedBaziService;
//# sourceMappingURL=enhancedBaziService.js.map