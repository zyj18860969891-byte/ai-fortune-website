...
- 健康状况：注意作息规律`;
    ...
    formatLuckyElements(elements) {
        ...
        // 确保元素是字符串数组
        return elements.map(element => typeof element === 'string' ? element : String(element)).filter((element, index, array) => array.indexOf(element) === index // 去重
        );
    }
    /**
     * 生成备选八字分析（当MCP服务不可用时）
     */
    generateFallbackBazi(request) {
