    ...
    async callTool(serverName, toolName, toolArgs) {
        try {
            ...
            if (!response.isError) {
                ...
                return {
                    success: true,
                    content: Array.isArray(response.content) ? response.content.map((c) => c?.text || '').join('\n') : '',
                    data: response
                };
            }
            else {
                ...
                return {
                    success: false,
                    error: `工具调用失败: ${Array.isArray(response.content) ? response.content.map((c) => c?.text || '').join('\n') : '未知错误'}`
                };
            }
        }
        catch (error) {
