...
import ModelScopeMcpService from './modelscopeMcpService';
...
export class ModelScopeService {
  ...
  private modelscopeMcpService: ModelScopeMcpService;
  private promptsConfig: any;
  ...
  constructor(config: { apiKey: string; modelId: string; baseUrl: string }) {
    ...
    // 初始化数据集服务
    this.datasetService = new DatasetService(this.apiKey);
    
    // 初始化ModelScope官方MCP服务
    this.modelscopeMcpService = new ModelScopeMcpService();
  ...
  async generateFortune(prompt: string, type: string, context?: string): Promise<{
    prediction: string;
    advice: string;
  ...
  private async getDatasetContent(prompt: string, type: string): Promise<string> {
    let relevantContent = '';
...
${randomItems.map((item: any, index: number) => 
  `- 知识点${index + 1}：${item?.content || item?.description || '专业命理知识'}`
).join('\n')}
            `;
          }
        }
...
相关塔罗知识：
${randomItems.map((item: any, index: number) => 
  `- 牌面解读${index + 1}：${item?.content || item?.description || '神秘塔罗指引'}`
).join('\n')}
            `;
          }
        }
  ...
  private extractKeywords(prompt: string, type: string): string[] {
  ...
  private buildFortunePrompt(question: string, type: string, datasetContent: string = '', context?: string): string {
  ...
  private getDefaultFortunePrompt(question: string, type: string, datasetContent: string = ''): string {
...
请用简洁有力的话语回答。`
    ...
    return defaultPrompts[type as keyof typeof defaultPrompts] || defaultPrompts.tarot;
  }
  ...
  private parseFortuneResponse(text: string, type: string): {
    prediction: string;
    advice: string;
    luckyElements: string[];
    confidence: number;
    ...
    const keywords = extractKeywords[type as keyof typeof extractKeywords] || extractKeywords.tarot;
  ...
  private getDefaultPrediction(type: string): string {
    const defaults = {
    ...
    return defaults[type as keyof typeof defaults] || defaults.tarot;
  }

  private getDefaultAdvice(type: string): string {
    const defaults = {
  ...
  private getDefaultLuckyElements(type: string): string[] {
    const defaults = {
      tarot: ['紫色', '7', '西方'],
      bazi: ['绿色', '3', '东方'],
      astrology: ['蓝色', '9', '北方'],
  ...
  private generateFallbackFortune(type: string): {
    prediction: string;
    advice: string;
    luckyElements: string[];
    confidence: number;
    ...
    return fallbacks[type as keyof typeof fallbacks] || fallbacks.tarot;
  }

  getConversationHistory(): { question: string; response: string }[] {
