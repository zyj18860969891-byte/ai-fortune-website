...
import { EnhancePromptService } from './EnhancePromptService';
...
export class RealModelScopeOnlineService {
  ...
  private enhancePromptService: EnhancePromptService;
  private lastApiCallTime: number = 0;
  private readonly MIN_API_INTERVAL: number = 3000; // 最小API调用间隔3秒

  constructor(config: RealModelScopeConfig) {
    this.config = config;
    this.enhancePromptService = EnhancePromptService.getInstance();
  ...
  private formatHumanLikeResponse(aiResponse: string, question: string): any {
  ...
  private generateFallbackResponse(question: string, type?: string, processingTime?: number): FortuneAnalysisResult {
    const responses = {
    ...
    const response = responses[type as keyof typeof responses] || responses.general;
   ...
   * 直接使用原始提示词，不进行任何增强
   */
  private async enhancePromptWithService(question: string, context?: string, systemPrompt?: string): Promise<string> {
    try {
...
请为用户进行详细的命理分析。`;
   ...
   */
  private buildRawPrompt(question: string, context?: string, systemPrompt?: string): string {
    // 完全模仿原生MCP服务的简单提示词
...
请基于八字命理知识直接回答用户的问题，不要泛泛而谈性格分析。`;
   ...
   */
  private buildEnhancedPrompt(question: string, context?: string, systemPrompt?: string): string {
...
请确保内容完整、专业且易于理解。`;
  ...
  private simplifyResponse(aiResponse: string): string {
    let cleanResponse = aiResponse.trim().replace(/^["']|["']$/g, '');
