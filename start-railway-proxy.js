...
app.use('/api', async (req, res) => {
  try {
    ...
    console.log(`🔄 原始请求: ${req.method} ${req.originalUrl}`);
    ...
    console.log(`🔄 响应数据长度:`, response.data ? JSON.stringify(response.data).length : 0);
    
    res.json(response.data);
    
  } catch (error) {
    console.error('❌ 代理请求失败:', error.message);
    console.error('❌ 错误详情:', error.response?.data || error.response?.status || error.code);
    
    // 如果后端不可用，返回智能本地响应
    if (error.code === 'ECONNREFUSED' || error.code === 'ETIMEDOUT') {
      console.log('🔄 后端服务不可用，使用智能本地响应');
      
      const { question, type, context, sessionId } = req.body || {};
      
      if (req.originalUrl.includes('/fortune/chat') && question) {
        // 生成智能本地八字分析
        ...
        res.json({
          ...
          source: 'intelligent-local-fallback',
...
app.get('/api/fortune/types', async (req, res) => {
  try {
    const response = await axios.get(`${BACKEND_URL}/api/fortune/types`);
    res.json(response.data);
  } catch (error) {
    console.error('获取运势类型失败:', error.message);
...
app.post('/api/fortune/chat', async (req, res) => {
  try {
    const response = await axios.post(`${BACKEND_URL}/api/fortune/chat`, req.body);
    res.json(response.data);
  } catch (error) {
